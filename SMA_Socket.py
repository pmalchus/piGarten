...
class SMASocket:
    ...
    def prepare_sending(self, cmd, sendersocket):
        ...
        data1 = codecs.decode(cmd, 'hex')

        len = int(cmd.__len__()/2)
        data = bytearray(len)
        count = 0
        for i in range(0, (len-1 *2), 2):
            msb = (int(cmd[i], 16) << 4)
            lsb = (int(cmd[i+1],16))
            byte = msb +lsb
